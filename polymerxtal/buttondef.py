        ...
        for line in outplala:
            ...
            if ord(line[0]) >= 97 and ord(line[0]) <= 123:
                ...
                if (
                    content[0] == "debug"
                    or content[0] == "ncn"
                    or content[0] == "standby"
                    or content[0] == "strachan"
                ):
        ...
        for cluster in ["halstead", "rice"]:
            for dirc in Dir[cluster]:
                if i <= Dir[cluster][dirc]["Max Walltime"]:
                    if (
                        eval(Dir[cluster][dirc]["Free"]) >= n
                        and eval(Dir[cluster][dirc]["Queue"]) == 0
                    ):
                    ...
                    if (
                        k == 2
                        and eval(Dir[cluster][dirc]["Free"]) >= n
                        and eval(Dir[cluster][dirc]["Queue"])
                        < eval(Dir[cluster][dirc]["Free"])
                    ):
                        if n != 16 and dirc == "debug":
                            continue
                        if (
...
def main(a, b, c):
    ...
    for line in src.readlines():
        ...
        for i in range(10):
            ...
            sentence = "stereo s1 "
            replace = (
                ("%s 2 " % ("weight" if tac == "ata" else "pattern"))
                + ln[0]
                + (" 0.5 " if tac == "ata" else " ")
                + ln[1]
                + (" 0.5" if tac == "ata" else "")
            ...
            os.system(
                "cat ../../../../input/title.in group.txt ../../../../input/link.in > link.in"
            )
            ...
            for j in ["Qeq"]:
                ...
                os.chdir(j)
                os.system(
                    "cat ../../../../../input/title.in cha.txt %snvt.in > nvt.in" % j
                )
                os.system("cp ../glance1.data polymer_relax.data")
                if j == "Qeq":
                    os.system("cp ../../../../../input/param.qeq.pan .")
                os.system(
                    "cat ../../../../../input/title.in cha.txt %snpt.in > npt.in" % j
                )
                # write_sublmp(tac,tit,i,j,'nvt.in')
                write_sublmp(
                    7200, "%s%d%s%s" % ("nvt", i, tit, tac), "nvt.in", tmp=0, te=""
                )
