...
def complete_args(args):
    ...
    polymer_types = [
        'PS-atactic', 'PS-isotactic', 'PS-syndiotactic', 'PE', 'PMMA-atactic', 'PMMA-isotactic', 'PMMA-syndiotactic', 'PP-atactic', 'PP-isotactic', 'PP-syndiotactic', 'POM',
        'PTFE', 'PVC'
    ]
    if 'polymer_type' in args:
        if len(args['polymer_type']) == 1:
            polymer_type = args['polymer_type'][0]
            if len(polymer_type.split(
                    '-')) == 1 and polymer_type != 'PE' and polymer_type != 'POM' and polymer_type != 'PTFE' and polymer_type != 'PVC':
    ...
    if flag_polymer_custom:
        ...
        if polymer_type_custom:
            ...
            molecular_weight = writepolymodfile(polymer_type_custom, 'polymer_types/custom/file.txt')
        else:
            return False
