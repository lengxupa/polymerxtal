...
def save_structure(file, outfilePDB=''):
    ...
    if extension == '.cif':
        os.system('obabel {0} -O {1} --fillUC strict --title convert ---errorlevel 1'.format(structure_path, outfilePDB))
    else:
        os.system('obabel {0} -O {1} --title convert ---errorlevel 1'.format(structure_path, outfilePDB))
...
def Create_Data_File(file, bondscale=1.1, lattice_in_file=False, ffield='Dreiding',charge='Gasteiger',xlo=0,xhi=50,ylo=0,yhi=50,zlo=0,zhi=50,alpha=90,beta=90,gamma=90):
