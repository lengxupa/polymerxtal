...
def getDREIDINGatomtypes(atoms, atomlinks, atomelems, atomlinkelems, atominring):
    ...
    for i in range(len(atomelems)):
        ...
        if atomelem == "H_":
            if (
                "O_" in atomlinkelems[i]
                or "N_" in atomlinkelems[i]
                or "S_" in atomlinkelems[i]
            ):
                atomtype[i] = "H__HB"
            else:
                atomtype[i] = "H_"
            ...
            if len(atomlinks[i]) <= 2:
                ...
                v1 = [
                    ...
                    (atoms[atomID1][6] - atoms[atomID0][6]),
                ]
                v2 = [
                    (atoms[atomID2][4] - atoms[atomID0][4]),
                    (atoms[atomID2][5] - atoms[atomID0][5]),
                    (atoms[atomID2][6] - atoms[atomID0][6]),
                ]
                v3 = [
                    (atoms[atomID3][4] - atoms[atomID0][4]),
                    (atoms[atomID3][5] - atoms[atomID0][5]),
                    (atoms[atomID3][6] - atoms[atomID0][6]),
                ]
                vx = cross(v2, v3)
                vp = dot(v1, vx)
                if abs(vp) < 1.0e-2:
...
def getPCFFatomtypes(atomlinks, atomelems, atomlinkelems, atominring, bondorder):
    ...
    for i in range(len(atomelems)):
        ...
        if atomelem == "H_":
            if (
                "C_" in atomlinkelems[i]
                or "SI" in atomlinkelems[i]
                or "H_" in atomlinkelems[i]
            ):
                atomPCFFtype[i] = "h"
            ...
            if len(atomlinks[i]) == 4:
                ...
                if atomlinkelems[i].count("N_") == 3:
                    ...
                    atomlinkelems[i].count("O_") == 2
                    and atomlinkelems[i].count("N_") == 1
                ):
                    atomPCFFtype[i] = "c_2"
                elif (
                    atomlinkelems[i].count("O_") == 1
                    and atomlinkelems[i].count("N_") == 2
                ):
                    atomPCFFtype[i] = "c_2"
                elif (
                    atomlinkelems[i].count("O_") == 2
                    and atomlinkelems[i].count("C_") == 1
                ):
                    atomPCFFtype[i] = "c_1"
                elif (
                    atomlinkelems[i].count("O_") == 1
                    ...
                    atomlinkelems[i].count("O_") == 1
                    and atomlinkelems[i].count("C_") == 2
                ):
                    atomPCFFtype[i] = "c_0"
                elif (
                    atomlinkelems[i].count("O_") == 1
                    and atomlinkelems[i].count("H_") == 1
                    and atomlinkelems[i].count("C_") == 1
                ):
                    atomPCFFtype[i] = "c_0"
                else:
            ...
            if len(atomlinks[i]) == 3:
                ...
                if (
                    ...
                    if (
                        ...
                        and atomlinkelems[CatomID - 1].count("N_") == 3
                    ):
                        atomPCFFtype[i] = "nr"
                    else:
                        atomPCFFtype[i] = "na"
            ...
            if len(atomlinks[i]) == 1:
                if (
                    atomlinkelems[i].count("O_") == 1
                    or atomlinkelems[i].count("N_") == 1
                ):
                    atomPCFFtype[i] = "o="
                elif (
                    atomlinkelems[i].count("S_") == 1
                    or atomlinkelems[i].count("P_") == 1
                ):
                    atomPCFFtype[i] = "o="
            ...
            if len(atomlinks[i]) == 2:
                ...
                if atomlinkelems[i].count("C_") == 2:
                    ...
                    atomlinkelems[i].count("SI") == 1
                    and atomlinkelems[i].count("H_") == 1
                ):
                    atomPCFFtype[i] = "osh"
                elif (
                    atomlinkelems[i].count("AL") == 1
                    and atomlinkelems[i].count("H_") == 1
                ):
                    atomPCFFtype[i] = "oah"
                elif (
                    atomlinkelems[i].count("AL") == 1
                    and atomlinkelems[i].count("SI") == 1
                ):
                    atomPCFFtype[i] = "oas"
                elif atomlinkelems[i].count("C_") == 1:
                    if atomlinkelems[i][0] == "C_":
...
############################################################
def atomTyping(
    forcefield, atoms, atomlinks, atomelems, atomlinkelems, atominring, bondorder
):
    if forcefield == "PCFF":
        newatomtypes = getPCFFatomtypes(
            atomlinks, atomelems, atomlinkelems, atominring, bondorder
        )
    else:
        newatomtypes = getDREIDINGatomtypes(
...
def doAtomTyping(forcefield):
    ...
    newatomtypes, atomwithnewtypeID = atomTyping(
        forcefield, atoms, atomlinks, atomelems, atomlinkelems, atominring, bondorder
    )
    atoms = updateAtoms(atoms, atomwithnewtypeID)
    # newatomtypes = map(remove_underscore, newatomtypes)
