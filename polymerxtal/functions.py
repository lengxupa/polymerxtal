...
def run_data4lammps(charge, ffname, cell):
    ...
    data4lammps_command = 'python {0} {1} {2} {3} {4} {5} {6} {7}'.format(path_r, 0, cell[0], 0, cell[1], 0, cell[2],
                                                                          charge)
...
def write_data(Dir, ofile, v=1, a=0):
    ...
    if a:
        ...
        List = [
            'Masses', 'Pair Coeffs', 'Bond Coeffs', 'Angle Coeffs', 'Dihedral Coeffs', 'Improper Coeffs', 'Atoms',
...
def input_coat5(cpos, radius, oradius, idata, odata, tstep, X6paircoeffs, mctemp):
    ...
    des.write('region outter cylinder z %f %f %f INF INF units box side in\n' % (cpos[0], cpos[1], oradius + 3.5))
    des.write('\n')
...
def add_data(Dir_1, Dir_2, add='append'):
    ...
    for i in ['atom', 'bond', 'angle', 'dihedral', 'improper']:
        ...
        if ('extra ' + i + ' per atom') in Dir_2:
            if ('extra ' + i + ' per atom') in Dir_data:
                Dir_data['extra ' + i + ' per atom'] = max(Dir_1['extra ' + i + ' per atom'],
                                                           Dir_2['extra ' + i + ' per atom'])
            else:
    ...
    List = ['Masses', 'Atoms', 'Velocities', 'Bonds', 'Angles', 'Dihedrals', 'Impropers']
    for key in List:
        if key in Dir_1 and len(Dir_1[key]) > 0:
            Dir_data[key] = Dir_1[key]
        if key in Dir_2 and len(Dir_2[key]) > 0:
            if key in Dir_data:
                if key in ['Masses', 'Atoms', 'Velocities', 'Bonds', 'Angles', 'Dihedrals', 'Impropers']:
                    if key == 'Atoms':
                        a_ids = {}
                    ...
                    for a_id in Dir_2[key]:
                        ...
                        if key == 'Atoms':
                            ...
                            Dir_data[key][ini + a_id][1] = Dir_2[key][a_id][1] + Dir_1['atom types']
                        if key == 'Bonds':
                            Dir_data[key][ini + a_id][0] = Dir_2[key][a_id][0] + Dir_1['bond types']
                        if key == 'Angles':
                            Dir_data[key][ini + a_id][0] = Dir_2[key][a_id][0] + Dir_1['angle types']
                        if key == 'Dihedrals':
                            Dir_data[key][ini + a_id][0] = Dir_2[key][a_id][0] + Dir_1['dihedral types']
                        if key == 'Impropers':
                            Dir_data[key][ini + a_id][0] = Dir_2[key][a_id][0] + Dir_1['improper types']
                        if key in ['Bonds', 'Angles', 'Dihedrals', 'Impropers']:
                            for i in range(1, len(Dir_2[key][a_id])):
                                Dir_data[key][ini + a_id][i] = a_ids[Dir_data[key][ini + a_id][i]]
            else:
    ...
    for key in ['Bond', 'Angle', 'Dihedral', 'Improper']:
        if (key + ' Coeffs') in Dir_polymer:
            for i in Dir_polymer[key + ' Coeffs']:
                if key == 'Bond':
                    des.write('bond_coeff %i harmonic %s\n' %
                              (i, ' '.join([str(k) for k in Dir_polymer[key + ' Coeffs'][i]])))
                elif key == 'Angle':
                    des.write('angle_coeff %i %s\n' % (i, ' '.join([str(k) for k in Dir_polymer[key + ' Coeffs'][i]])))
                elif key == 'Dihedral':
                    des.write('dihedral_coeff %i %s\n' %
                              (i, ' '.join([str(k) for k in Dir_polymer[key + ' Coeffs'][i]])))
                elif key == 'Improper':
                    des.write('improper_coeff %i %s\n' %
                              (i, ' '.join([str(k) for k in Dir_polymer[key + ' Coeffs'][i]])))
    des.close()
            ...
            for j in range(polymer_type_custom['monomer'][i + 1]['torsion']['len'] - 2):
                if str(j + 3) in polymer_type_custom['monomer'][i + 1]['torsion']:
                    ...
                    print(
                        ...
                        % ((j + 3), (i + 1)))
                    tor_type = 0
                    des.write('%d                    # Torsion %d specification: no change\n' % (tor_type, (j + 3)))
        des.write('\n')
...
def main(args):
    ...
    if 'p_fra' in args:
        if len(args['p_fra']) == 1:
            ...
            if p_fra < 0 or p_fra > 1:
                ...
                print('Molecular weight fraction of polymer coated: ' + str(p_fra * 100) + '%')
        else:
    ...
    # Run polymod
    shift, cell_polymod, cpos, caxis, clength = parameters(periodic, cell_sizes, coord, boundaries, radius)
    ...
    while p_flag:
        ...
        write_minimize('minimize.in', 'str2lammps/X6paircoeffs.txt', 'str2lammps/types/%s_atom_type.dat' % p)
        ...
        for i in range(10):
            ...
            input_coat4(na, cpos, 'step%d.data' % (i * 3 + 1), 'step%d.data' % (i * 3 + 2))
            ...
            input_coat5(cpos, radius - 5, oradius, 'step%d.data' % (i * 3 + 3), 'step%d.data' % (i * 3 + 4), i * 5000,
                        'str2lammps/X6paircoeffs.txt', mctemp)
    ...
    for i in range(10):
        ...
        input_coat7(oradius, 'step%d.data' % (i * 3 + 32), 'step%d.data' % (i * 3 + 33), i * 5000,
                    Dir_polymer['atom types'], len(elementlist), mctemp)
