...
lattice = ui.Number(name='Lattice Constant',
    desc='''In the single-band model considered here, this constant represents the discretization spacing of a simple cubic lattice. The shape of the quantum dot will be composed of integer multiples of this quantity.
...
emass = ui.Number(name='Effective Mass',
...
single_options = ui.Form([nstates, shape, shape_image, dotx, doty, dotz, lattice, emass, Eg], name='Simple Quantum Dot Options')
...
qds = ui.Dropdown(
    name='Type of Quantum Dot Structure',
...
theta = ui.Number(name='Angle Theta',
    desc='Angle of the light source with respect to the vertical axis (z-axis) of the quantum dot, in degrees.  The value theta=0 is shining straight down on the quantum dot.  The value theta=90 is shining parallel to the x-y plane.',
    min=0,
    max=90,
    value=45,
    units='deg'
)
phi = ui.Number(name='Angle Phi',
    desc='Angle of the light source around the quantum dot, in degrees.  This is the angle of rotation around the vertical axis--i.e., around the z-axis, in the x-y plane.',
    min=0,
    max=360,
    value=0,
    units='deg'
...
absolute_Ef = ui.Checkbox('Absolute Fermi Level', 
    desc='Setting this to true makes the Fermi level an absolute energy value. If this option is not enabled, then Fermi level value is taken with respect to the lowest eigenstate. For example, if Ef = 0 eV, then it will correspond to the lowest eigenenergy (which could be 0.5eV for example).',
    value=False
)
Ef = ui.Number('Electron Fermi Level',
    desc='Input field for the position of the Fermi energy relative to the lowest energy state. States below the Fermi level are occupied with electrons, and can therefore be excited to the unoccupied states above the Fermi level.',
    min=-10,
    max=10,
    value=0,
    units='eV'
...
sweep = ui.Dropdown(
    ...
    options=["Fermi level in units of 'eV'", "Temperature in units of 'K'", "Angle theta in units of 'degree'", "Angle phi in units of 'degree'"],
)

sweep_min = ui.Number('Minimum',
    desc='Minimum value of the parameter being swept.', 
    value=0
)
sweep_max = ui.Number('Maximum',
    desc='Maximum value of the parameter being swept.', 
    value=90
)
npts = ui.Integer('Number of Points',
    desc='Number of points between min/max in the sweep.',
    min=2,
    max=20,
    value=3
)
