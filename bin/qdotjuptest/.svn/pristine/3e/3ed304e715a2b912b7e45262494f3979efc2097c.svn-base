...
QDOT_UI['nstates'] = ui.Integer(name='Number of States',value=inputs.nstates.value,min=inputs.nstates.min,max=inputs.nstates.max,units=inputs.nstates.units)
...
QDOT_UI['dotx'] = ui.Number(name='X Dimensions',value=inputs.dotx.value,min=inputs.dotx.min,max=inputs.dotx.max,units=inputs.dotx.units )
QDOT_UI['dotx'].dd.observe(lambda obj : UI_SET_VALUE(inputs.dotx,obj.new), names='value')
QDOT_UI['doty'] = ui.Number(name='Y Dimensions',value=inputs.doty.value,min=inputs.doty.min,max=inputs.doty.max,units=inputs.doty.units )
QDOT_UI['doty'].dd.observe(lambda obj : UI_SET_VALUE(inputs.doty,obj.new), names='value')
QDOT_UI['dotz'] = ui.Number(name='Z Dimensions',value=inputs.dotz.value,min=inputs.dotz.min,max=inputs.dotz.max,units=inputs.dotz.units )
QDOT_UI['dotz'].dd.observe(lambda obj : UI_SET_VALUE(inputs.dotz,obj.new), names='value')
QDOT_UI['lattice'] = ui.Number(name='Lattice Constant',value=inputs.lattice.value,min=inputs.lattice.min,max=inputs.lattice.max,units=inputs.lattice.units )
QDOT_UI['lattice'].dd.observe(lambda obj : UI_SET_VALUE(inputs.lattice,obj.new), names='value')
QDOT_UI['emass'] = ui.Number(name='Effective Mass',value=inputs.emass.value,min=inputs.emass.min,max=inputs.emass.max,units=inputs.emass.units )
QDOT_UI['emass'].dd.observe(lambda obj : UI_SET_VALUE(inputs.emass,obj.new), names='value')
QDOT_UI['Eg'] = ui.Number(name='Energy Gap',value=inputs.Eg.value,min=inputs.Eg.min,max=inputs.Eg.max,units=inputs.Eg.units )
QDOT_UI['Eg'].dd.observe(lambda obj : UI_SET_VALUE(inputs.Eg,obj.new), names='value')

QDOT_UI['theta'] = ui.Number(name='Angle Tetha',value=inputs.theta.value,min=inputs.theta.min,max=inputs.theta.max,units=inputs.theta.units )
QDOT_UI['theta'].dd.observe(lambda obj : UI_SET_VALUE(inputs.theta,obj.new), names='value')
QDOT_UI['phi'] = ui.Number(name='Angle Phi',value=inputs.phi.value,min=inputs.phi.min,max=inputs.phi.max,units=inputs.phi.units )
...
QDOT_UI['Ef'] = ui.Number(name='Electron Fermi Level',value=inputs.Ef.value,min=inputs.Ef.min,max=inputs.Ef.max,units=inputs.Ef.units )
QDOT_UI['Ef'].dd.observe(lambda obj : UI_SET_VALUE(inputs.Ef,obj.new), names='value')
QDOT_UI['temperature'] = ui.Number(name='Temperature',value=inputs.temperature.value,min=inputs.temperature.min,max=inputs.temperature.max,units=inputs.temperature.units )
...
QDOT_UI['smin'] = ui.Number(name='Minimum',value=inputs.smin.value,min=inputs.smin.min,max=inputs.smin.max,units=inputs.smin.units )
QDOT_UI['smin'].dd.observe(lambda obj : UI_SET_VALUE(inputs.smin,obj.new), names='value')
QDOT_UI['smax'] = ui.Number(name='Maximum',value=inputs.smax.value,min=inputs.smax.min,max=inputs.smax.max,units=inputs.smax.units )
QDOT_UI['smax'].dd.observe(lambda obj : UI_SET_VALUE(inputs.smax,obj.new), names='value')
QDOT_UI['spts'] = ui.Integer(name='Number of Points',value=inputs.spts.value,min=inputs.spts.min,max=inputs.spts.max)
...
def fig_Angle_polarized( execution ):
    layout = { 
        ...
        'title': 'Light and Dark Transitions (phi='+str(inputs.phi.value)+' theta='+str(inputs.theta.value)+'))',
    }
...
def on_button_clicked(b):
    ...
    with QDOT_UI['sim_progress']:
        ...
        for l in range(inputs.nstates.value):
            ...
            but2.on_click(lambda event, state=l, param=r.db, func=fig_Energy_value, out=output: showFig(func, param, out, state))
            buttons.append(but2)
        ...
        new_box = HBox([VBox(buttons, layout=Layout(width='150px')), VBox([output], layout=Layout(width='99%'))], layout=Layout(width='99%'))
