...
def getAtomtypes():
    ...
    for line in fin:
        words=string.split(line[0:len(line)-1])
...
def getAtoms():  
    ...
    for i in range(totalatoms):
        ...
        nmolecule=eval(string.strip(str(words[1])))
...
def getPCFFatomtypes(atomlinks,atomelems,atomlinkelems,atominring,bondorder):
    ...
    for i in range(len(atomelems)):
        ...
        if atomelem=="H_":
            ...
            atomPCFFtype[i]=atomelem.capitalize()
    return atomPCFFtype    
############################################################
...
def outputTyping(forcefield,atomtypes,bondtypes,atoms,bonds):
    fout=open("forcefield.name",'w')
    print >>fout,forcefield
    fout.close()
    
    fout=open("./types/newatom_type.dat",'w')
    for i in range(len(atomtypes)):
        ntype=i+1
        atype=atomtypes[i]
        print >>fout,ntype,atype
    fout.close()
    
    fout=open("./types/newbond_type.dat",'w')
    for i in range(len(bondtypes)):
        ntype=i+1
        ...
        print >>fout,ntype,a1type,a2type
    fout.close()
    
    outfile="./types/newatoms.dat"
    fout=open(outfile,'w')
    print >>fout,len(atoms)," Atoms"
    print >>fout
    print >>fout,"Atoms"
    print >>fout
    for i in range(len(atoms)):
